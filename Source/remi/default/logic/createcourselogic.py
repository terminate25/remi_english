...
class CreateCourseLogic:
    ...
    def save_course_video(request, part_id, filename):
        upload_video_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL_RELATIVE, part_id, 'video')
    ...
    def save_lesson_summary(request, lesson_id, filename):
        ...
        upload_pdf_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL_RELATIVE, lesson_id, 'summary')
    ...
    def store_question_list(request,  part_id):
        ...
        for i in range(0, len(list_testid)):
            ...
            test_dir = os.path.join(settings.BASE_DIR, settings.MEDIA_URL_RELATIVE, str(part_id), 'test', str(list_testid[i]))
        ...
        for i in range(0, len(list_questionid)):
            ...
            question_dir = os.path.join(settings.BASE_DIR, settings.MEDIA_URL_RELATIVE, str(part_id), 'test', str(test_id), str(list_questionid[i]))
    ...
    def store_question_files(request, question_info):
        ...
        if question_type == QuestionType.Type1.code:
            ...
            if audio_file is not None:
                ...
                question_file_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test', str(test_id),
            ...
            if list_question_change is not None:
                if str(question_id) in list_question_change:
                    answer_dir = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test', str(test_id), str(question_id), 'answer')
            ...
            for image in images:
                image_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test',
            ...
            if list_question_change is not None:
                if str(question_id) in list_question_change:
                    answer_dir = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test',
                                              str(test_id), str(question_id), 'answer')
                    if os.path.exists(answer_dir):
                        shutil.rmtree(answer_dir, ignore_errors=True)
            ...
            for audio in audios:
                audio_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test',
                                          str(test_id),
            ...
            if audio_file is not None:
                ...
                #save ne
                question_file_name = audio_file.name
                upload_video_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test', str(test_id),
                                                 str(question_id), 'question')
            ...
            path_save_qf = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test',
                                        str(test_id),
                                        str(question_id), 'question')
            ...
            #save answer
            path_save_af = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(lesson_id), 'test',
                                        str(test_id),
                                        str(question_id), 'answer')
    ...
    def delete_part(part_id):
        ...
        lesson_dir = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(part_id))
        ...
        for i in range(0, test_info.count()):
            ...
            for j in range(0, question_info.count()):
                ...
                if type_test != 5:
                    ...
                    question_file_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(part_id), 'test',
                ...
                #get answer in folder
                    ...
                    answer_file_path = os.path.join(settings.BASE_DIR, settings.MEDIA_URL.replace('/', ''), str(part_id), 'test', str(test_info[i].id), str(question_info[j].id), 'answer')
